...
////// Lighting:
////// Emissive:
                float4 _HDRImage_var = texCUBE(_HDRImage,viewReflectDirection);
                ...
                float node_9430 = 1.0;
                float3 emissive = (lerp((_Multiply*(_HDRImage_var.rgb*_SkyColor.rgb)),(_Multiply*(_HorizonColor.rgb*_HDRImage_var.rgb)),pow((1.0 - max(0,dot(viewDirection,float3(0,-1,0)))),8.0))+(_LightColor0.rgb*pow(saturate((node_9430 + ( (max(0,dot((-1*lightDirection),viewDirection)) - node_1476) * (0.0 - node_9430) ) / ((1.0 - (node_9367*node_9367)) - node_1476))),5.0)*_SunIntensity));
